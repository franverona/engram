  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 animate-[slideUp_0.2s_ease-out] rounded-lg border border-border bg-surface px-4 py-3 shadow-lg">
      <div className="flex items-center gap-3">
        ...
        {onUndo && (
          <button onClick={dismiss} className="ml-2 text-sm border bg-black/50 border-border px-2 py-1 rounded-lg text-text-faint hover:text-foreground">
            Undo
          </button>
        )}
...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const showToast = useCallback((message: string, onUndo?: () => void) => setToast({ message, onUndo }), [])
